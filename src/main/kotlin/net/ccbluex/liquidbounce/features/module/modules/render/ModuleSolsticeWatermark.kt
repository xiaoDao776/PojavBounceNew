...
object ModuleSolsticeWatermark : ClientModule(
    ...
    private val glowRadius by float("Glow Radius", 18f, 2f..28f)
    private val glowStrength by float("Glow Strength", 0.4f, 0f..1f)
    private val glowSoftness by float("Glow Softness", 6f, 2f..12f)
    private val animSpeed by float("Anim Speed", 10f, 1f..30f)
    private val charSpacing by float("Char Spacing", 1.2f, -2f..4f)
    private val themeSpeed by float("Theme Speed", 0.9f, 0.1f..5f)
